...
// if(! empty($recaptcha)):
    ...
    $secret = '6Lc92XYUAAAAANRc1k5vhCuqljJFvuGfXyTeew8R';
    ...
    if($responseData->success):
        if($post):
            ...
            if(! $error):
                ...
                $mail->Host = 'mail.cdl.med.br';                          // Specify main and backup SMTP servers
                $mail->SMTPAuth = true;                                 // Enable SMTP authentication
                $mail->Username = 'sac@cdl.med.br';                      // SMTP username
                $mail->Password = 'cdl102030';                 // SMTP password
                $mail->SMTPSecure = 'tls';                              // Enable TLS encryption, `ssl` also accepted
                $mail->Port = 2525;                                      // TCP port to connect to

                $mail->setFrom('sac@cdl.med.br', 'Your Company Name');                  // From email address
                $mail->addAddress('sac@cdl.med.br', 'Recipient Name');                    // Add a recipient
                $mail->addAddress('sac@cdl.med.br', 'Secondary Recipient Name');     // Add another (optional) recipient
