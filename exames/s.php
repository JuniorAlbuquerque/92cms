...
if (!function_exists('GetSQLValueString')) {
    function GetSQLValueString(
        ...
        $theValue = function_exists('mysqli_real_escape_string')
            ? mysqli_real_escape_string($conn92ID, $theValue)
            : mysqli_escape_string($conn92ID, $theValue);
...
$colname_rsExames = '-1';
if (isset($_GET['name'])) {
    $colname_rsExames = $_GET['name'];
}
mysqli_select_db($conn92ID, $database_conn92ID);
$query_rsExames = sprintf(
    'SELECT * FROM tb_exames WHERE EXA_NOME LIKE %s ORDER BY EXA_NOME ASC',
    GetSQLValueString($colname_rsExames . '%', 'text')
);
($rsExames = mysqli_query($conn92ID, $query_rsExames)) or
    die(mysql_error($conn92ID));
...
<meta name="keywords" content="laboratório, laboratórios, exames, m�dicos, cl�nicas, sa�de, hospitais, medicina, manaus, amazonas">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div id="accordionOne" class="accordion accordion--style-3 accordion-cards--stacked" role="tablist" aria-multiselectable="true">
                                        ...
                                        <?php do { ?>
                                        <div class="card">
                                            <div class="card-header" role="tab" id="accordionOneHeading-<?php echo $row_rsExames[
                                                'EXA_COD'
                                            ]; ?>">
                                              <h5 class="card-title mb-0">
                                                <a class="collapsed" data-toggle="collapse" data-parent="#accordionOne" href="#accordionOneCollapse-<?php echo $row_rsExames[
                                                    'EXA_COD'
                                                ]; ?>" aria-controls="accordionOneCollapse-<?php echo $row_rsExames[
    'EXA_COD'
]; ?>">
                                                  <?php echo $row_rsExames[
                                                      'EXA_NOME'
                                                  ]; ?>
                                                  </a>
                                                </h5>
                                            ...
                                            <div id="accordionOneCollapse-<?php echo $row_rsExames[
                                                'EXA_COD'
                                            ]; ?>" class="collapse" role="tabpanel" aria-labelledby="accordionOneHeading-<?php echo $row_rsExames[
    'EXA_COD'
]; ?>">
                                          ...
                                          <?php } while (
                                            $row_rsExames = mysqli_fetch_assoc(
                                                $rsExames
                                            )
                                        ); ?>
<!-- /item -->
...
<!-- InstanceEnd --></html>
<?php mysqli_free_result($rsExames);
?>
